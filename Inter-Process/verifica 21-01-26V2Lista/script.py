...
class GUI:
    ...
    def cm_estrai(self):
        nome = self.input.get()
        if not nome:
            self.log_text.insert(tk.END, "Inserisci un nome per rimuovere\n")
            return
        risposta = java("estrai", nome)
        if risposta:
            resp = risposta.strip()
            if resp != "VUOTO":
                self.log_text.insert(tk.END, "Estratto " + resp + '\n')
                # ripristina l'interfaccia dai dati nel backend
                # (ricostruisce i nodi attuali)
                self.nodi.destroy()
                self.nodi = tk.Frame(self.frame)
                self.nodi.pack()
                self.buttons = []
                ...
                self.log_text.insert(tk.END, "Nome non trovato o coda vuota\n")
        else:
